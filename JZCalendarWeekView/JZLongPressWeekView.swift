...
open class JZLongPressWeekView: JZBaseWeekView {
	...
	*/
	...
	override open func getDateForPointX(_ xCollectionView: CGFloat) -> Date {
		...
		let section = Int(adjustedX / currentPageSectionWidth())
		let pageSectionX = flowLayout.pageSectionXs.first(where: {
			$0.value.0 <= xCollectionView && $0.value.1 > xCollectionView
		})
		print("section", pageSectionX?.key)
		return getDateForSection(pageSectionX!.key)
	}
