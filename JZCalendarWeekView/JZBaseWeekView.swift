...
public typealias MyDataSource = [Date: [[AppointmentEvent]]]
...
open class JZBaseWeekView: UIView {
	...
	*/
	...
	public var pageToSectionsMap: [Page: [Int]] = [:]
	...
	var pageDates: [Page: Date] {
		return [
			.previous: self.initDate,
			.current: self.initDate.add(component: .day, value: 1),
			.next: self.initDate.add(component: .day, value: 2)
		]
	}
	...
	public var allEventsBySection: MyDataSource!
	...
	*/
	open func setupCalendar(
													setDate: Date,
													allEvents: MyDataSource,
													scrollType: JZScrollType = .pageScroll,
													firstDayOfWeek: DayOfWeek? = nil,
													currentTimelineType: JZCurrentTimelineType = .page,
	...
	open func updateWeekView(to date: Date) {
		self.initDate = date.startOfDay.add(component: .day, value: -1)
		DispatchQueue.main.async { [unowned self] in
			self.layoutSubviews()
			self.forceReload()
		}
	...
	open func scrollWeekView(to time: Date) {
		...
		collectionView.setContentOffsetWithoutDelegate(CGPoint(x: collectionView.contentOffset.x, y: max(0, min(setTimeY, maxOffsetY))), animated: false)
	}
	
	/// Get current event with item indexPath
	///
	/// - Parameter indexPath: The indexPath of an item in collectionView
	open func getCurrentEvent(with indexPath: IndexPath) -> AppointmentEvent? {
		let date = flowLayout.dateForColumnHeader(at: indexPath.section)
		let appointments = allEventsBySection[date]
	...
	*/
	open func refreshWeekView() {
		updateWeekView(to: self.initDate.add(component: .day, value: 1))
	}
	...
	open func updateFirstDayOfWeek(setDate: Date, firstDayOfWeek: DayOfWeek?) {
		...
		self.initDate = setDate.startOfDay.add(component: .day, value: -1 - diff)
...
//	func numberOfSections(pageIdx: Int) -> Int {
//
//	}
	...
	open func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
		let date = flowLayout.dateForColumnHeader(at: section)
		if let eventsByDate = allEventsBySection[date] {
	...
	open func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
		...
		case JZSupplementaryViewKinds.columnHeader:
			if let columnHeader = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: kind, for: indexPath) as? JZColumnHeader {
				columnHeader.updateView(date: flowLayout.dateForColumnHeader(at: indexPath.section))
				view = columnHeader
			}
...
extension JZBaseWeekView: UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
	...
	private func paginationEffect(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
		...
		let pageWidth: CGFloat = scrollType == .sectionScroll ? contentViewWidth : contentViewWidth
		// current section will always get current section, applied floor Int here
		let currentSection = Int(currentContentOffset.x / contentViewWidth)
		let currentPage = scrollType == .sectionScroll ? currentSection : (currentSection >= 1 ? 1 : 0)  // The divider section for 0 and 1 page is at numOfDays
...
extension JZBaseWeekView {
	...
	public func setHorizontalEdgesOffsetX() {
		let currentPageFirstDate = initDate.add(component: .day, value: 1)
		let currentPageLastDate = initDate.add(component: .day, value: 1)
			...
			if let startDate = scrollableRange.startDate {
				if startDate >= currentPageFirstDate {
					...
					if scrollType == .pageScroll || startDate <= firstDateInView {
						...
						scrollableEdges.leftX = contentViewWidth * CGFloat(days)
					}
				}
			} else {
				scrollableEdges.leftX = nil
		...
		if let startDate = scrollableRange.startDate, startDate > currentPageLastDate {
			...
			if let endDate = scrollableRange.endDate {
				if endDate <= currentPageLastDate {
					...
					let lastDateInView = initDate.add(component: .day, value: 2)
					if scrollType == .pageScroll || endDate >= lastDateInView {
						...
						scrollableEdges.rightX = contentViewWidth * CGFloat(days)
					}
				}
			} else {
				scrollableEdges.rightX = nil
...
//	private func getDate(section: IndexPath) -> Date {
...
//			let upper = lower + byDate.value.count
//			if lower <= section && upper > section {
//				return (idx, section - lower)
//			} else {
//				previousUpper = upper
//			}
//		}
...
//		pages.mapValues { pageDate in
//			let pageEvents = events[pageDate]
//		}
		return result
	}
